...
exports.registerHandle = async (req, res) => {
    ...
    //------------ Checking Captcha ------------//
    const secretKey = '6LfsrZUfAAAAANKEC19zUjHC42p4TIRRMZf4RHnG';

    if(!captcha){
        errors.push({ msg: 'Please verify captcha' });
        res.render('register', {
            errors,
            name,
            email,
    ...
    if (body.success !== undefined && !body.success){
        errors.push({ msg: 'Could not verify captcha' });
        res.render('register', {
            errors,
            name,
            email,
    ...
    if (errors.length > 0) {
        ...
        User.findOne({ email: email }).then(user => {
            if (user) {
                ...
                const token = jwt.sign({ name, email, password, mobile }, JWT_KEY, { expiresIn: '30m' });
    ...
    if (token) {
        jwt.verify(token, JWT_KEY, (err, decodedToken) => {
            if (err) {
                ...
                const { name, email, password, mobile } = decodedToken;
                User.findOne({ email: email }).then(user => {
                    if (user) {
                        //------------ User already exists ------------//
                        req.flash(
